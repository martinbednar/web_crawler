...
(function () {
    ...
    const recordBlockedFeature = (blockReport, tabId, frameId, featurePath) => {
         ...
         * Marek Schauer
         * ---------------
         * This is a place, where we know about all method calls. 
         * We will log them from there.
         * 
         * From this place, we will emit the message, something like this:
        ...
        if (standardReportsForFrame[standardId] === undefined) {
            // console.log("Standard id = " + standardId + ' is undefined');
            standardReportsForFrame[standardId] = {};
        } else {
            // console.log("Standard id = " + standardId + ' is OK');
        }

        // console.log('Standard id = ' + standardId + ':', standardReportsForFrame[standardId]);
        
        if (standardReportsForFrame[standardId][featurePath] === undefined) {
            // console.log('Also the feature path is undefined, so I am creating new feature path: ' + featurePath);
            standardReportsForFrame[standardId][featurePath] = 1;
        } else {
            // console.log('Woohoo! Feature path ' + featurePath + ' exists!');
            standardReportsForFrame[standardId][featurePath] += 1;
        }
        ...
        //     if (xmlHttpObj.readyState == XMLHttpRequest.DONE) {   // XMLHttpRequest.DONE == 4
        //         if (xmlHttpObj.status == 200) {
        //             console.log('Recieved following response from logging server: ', xmlHttpObj.responseText);
        //         }
        //         else {
        //             console.log('Some error occured while logging into server, return code =', xmlHttpObj.status);
        //         }
        //     }
        // };

        // xmlHttpObj.open("POST", "http://www.stud.fit.vutbr.cz/~xschau00/others/SEP/API/test.php?q=i_am_just_logging_some_feature", true);
        // xmlHttpObj.send();

        // console.log('Current state of logging object:', blockReport);
        // console.log('<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<');
    };
