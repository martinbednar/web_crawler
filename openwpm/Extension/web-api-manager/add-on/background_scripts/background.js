...
(async function () {
    "use strict";
    const {preferencesLib, constants, messagesLib, standardsLib} = window.WEB_API_MANAGER;
    const {cookieEncodingLib, proxyBlockLib, httpHeadersLib} = window.WEB_API_MANAGER;
    const {loggingDB, browserLib, enums} = window.WEB_API_MANAGER;
    const {jsInstrumentLib} = window.WEB_API_MANAGER;
    const rootObject = browserLib.getRootObject();

    // Let's load config information from profile dir, 
    // saved there by OpenWPM
    let filename = "browser_params.json";
    let openWPMConfig = await browser.profileDirIO.readFile(filename);
    console.log(openWPMConfig);
    // Let's create loggingDB
    const config = JSON.parse(openWPMConfig);
    ...
    // ---------------------------------------
    // ---------------------------------------
    console.log('Let\'s create the JavascriptInstrument object 🤓');
    loggingDB.logDebug("Logging from web api manager enabled");
    let jsInstrument = new jsInstrumentLib.JavascriptInstrument(loggingDB);
    ...
    // let sendingSocket = new socketsLib.SendingSocket();
    // sendingSocket.connect("127.0.0.1", 43555);
    // console.log(sendingSocket);
    ...
    rootObject.webRequest.onHeadersReceived.addListener(details => {
        ...
        // ******************************
        const standardIdsToBlock = standardsLib.allStandardIds();
        console.log('When all standards are checked, this is how the standardIdsToBlock looks like:', matchingRule.getStandardIds());
        console.log('... and this is how it looks like when i am trying to retrieve standardsLib.allStandardIds():', standardsLib.allStandardIds());
        const customBlockedFeatures = matchingRule.getCustomBlockedFeatures();
        // const shouldLog = prefs.getShouldLog();
        const shouldLog = enums.ShouldLogVal.STANDARD;
