...
(function () {
    ...
    const proxyBlockingFunction = function () {
        ...
        const createBlockingProxy = keyPath => {
            ...
            const pathToRef = keyPath => {
                if (keyPath) {
                    // console.log('--->', keyPath, '<---');
                    const keyPathArr = keyPath.split('.');
        
                    return keyPathArr.reduce(function (prev, cur) {
                        if (prev === undefined) {
                            return undefined;
                        }
                        return prev[cur];
            ...
            const blockingProxy = new Proxy(pathToRef(keyPath), handlers);
    ...
    const generateScriptPayload = blockingSettings => {
        ...
        console.log('blockingSettings: ', blockingSettings);
